...
<div id="main-wrapper" class="container">
	...
	<article id="content" <?php post_class( $content_class ); ?>>
		...
		<div id="review-summary-wrapper">
			<?php
			// if ( is_single( 'solar-link-australia' ) )
			// {
			// 	get_template_part( 'templates/parts/company-rating-summary-aggregation' );
			// }
			// else
			// {
			// 	get_template_part( 'templates/parts/company-rating-summary' );
			// }
...
//		$content = strip_tags( $content );
		...
		$content = str_replace( array( "\n", '<br><br>' ), 'RILWIS ', $content );
		$content = preg_split( '/\s+/', $content );

		// Get first text part and "read more" part
		$first   = implode( ' ', array_slice( $content, 0, 100 ) );
		$first   = force_balance_tags( $first );
		$first   = str_replace( 'RILWIS ', '<br><br>', $first );
		$second  = implode( ' ', array_slice( $content, 100 ) );
		$second  = force_balance_tags( $second );
		$second   = str_replace( 'RILWIS ', '<br><br>', $second );

		// Concatenate content
		if ( $second )
			$content = $first . '<a href="#" class="show-content" id="show-content">' . __( 'Read more', '7listings' ) . '</a>' . '<div class="more-content" id="more-content">' . $second . '</div>';
		else
			$content = $first;

		$content = str_replace( 'RILWIS', '', $content );
		...
		if ( ! is_wp_error( $terms ) && ! empty( $terms ) )
		{
			...
			foreach ( $terms as $term )
			{
				...
				if ( $count == 10 )
				{
					echo '<a href="#" class="show-brands" id="show-brands">' . __( 'Show all brands', '7listings' ) . '</a>';
					echo '<div class="other-brands" id="other-brands">';
				}
			}
			if ( $count > 10 )
				echo '</div>'; // .other-brands
		...
		if ( !is_wp_error( $terms ) && !empty( $terms ) )
		{
			...
			foreach ( $terms as $term )
			{
				if($count < 5)
					...
					printf( '<li class="other-products" itemprop="owns" itemscope itemtype="http://schema.org/Product"><a href="%s" title="%s" itemprop="name">%s</a></li>', get_term_link( $term, $tax ), $term->name, $term->name );

				$count++;
			}
			echo '</ul>';
			if($count > 5)
				echo '<a href="#" class="show-all-product" id="show-all-product">' . __( 'Show all products', '7listings' ) . '</a>';
			echo '</section>';
		}
		...
		if ( !is_wp_error( $terms ) && !empty( $terms ) )
		{
			...
			foreach ( $terms as $term )
			{
				if($count < 5)
					...
					printf( '<li class="other-services" itemprop="owns" itemscope itemtype="http://schema.org/Product"><a href="%s" title="%s" itemprop="name">%s</a></li>', get_term_link( $term, $tax ), $term->name, $term->name );
				$count++;
			}
			echo '</ul>';
			if($count > 5)
				echo '<a href="#" class="show-all-service" id="show-all-service">' . __( 'Show all services', '7listings' ) . '</a>';
			echo '</section>';
		}
