	...
	<div id="main-wrapper" class="container">
		...
		<article id="content" <?php post_class( $content_class ); ?>>
			...
			if ( sl_setting( get_post_type() . '_single_features' ) )
			{
				...
				the_terms( get_the_ID(), 'features', '<h3>' . __( 'Attraction Features', '7listings' ) . '</h3>', ', ', '' );
				echo '</section>';
			}
			?>
			...
			if ( current_user_can( 'manage_options' ) )
				echo '<span class="edit-link button small page-settings ic-only"><a class="post-edit-link" href="' . admin_url( 'edit.php?post_type=page&page=attraction#single_settings' ) . '" title="' . __( 'Edit Page Settings', '7listings' ) . '">' . __( 'Edit Page Settings', '7listings' ) . '</a></span>';
			?>
 ...
 * @return void
 */
function sl_attraction_rebuild_args( $not_duplicated, &$args, &$type_args, &$price_args )
{
 ...
 * @return void
 */
function sl_similar_attraction( $query, $limit = 3 )
{
