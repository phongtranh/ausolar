 ...
 * @return void
 */
function sl_homepage_show_tour_modules( $id )
{
	...
	switch ( $id )
	{
		...
		case 'tour_featured':
			...
			// We set only some settings here. Some ones don't need normalizing as they'll be set by Sl_Tour_Frontend::slider()
			...
			// Change some settings name to match Sl_Tour_Frontend::slider() arguments
			$module_settings['number'] = $module_settings['total'];
			unset( $module_settings['total'] );

			if ( $title = $module_settings['title'] )
			{
				$heading_style  = sl_heading_style( 'homepage_tour_featured_title' );
				$title          = "<" . $heading_style . " class='title section'>{$title}</" . $heading_style . ">";
			}

			echo '<section id="tours-slider" class="container">' . $title;
			echo Sl_Tour_Frontend::slider( $module_settings );
			echo '</section>';
			break;
		...
		case 'tour_listings':
			// Normalize settings, make sure settings are set
			// We set only some settings here. Some ones don't need normalizing as they'll be set by Sl_Tour_Frontend::post_list()
			$module_settings = array_merge( array(
				'title'     => '',
				'display'   => 12,
				'layout'    => 'grid',
			...
			// Change some settings name to match Sl_Tour_Frontend::post_list() arguments
			...
			if ( $title = $module_settings['title'] )
			{
				$heading_style  = sl_heading_style( 'homepage_tour_listings_title' );
				$title = "<" . $heading_style . " class='title section'>{$title}</" . $heading_style . ">";
			}

			echo '<section id="tours-list" class="container">' . $title;
			echo Sl_Tour_Frontend::post_list( $module_settings );
			echo '</section>';
			break;
