 ...
 */
class FitWP_Bootstrap_Nav_Walker extends Walker_Nav_Menu
{
	/**
 ...
 */
class FitWP_Bootstrap_Page_Walker extends Walker_Page
{
	/**
	...
	function start_el( &$output, $page, $depth = 0, $args = array(), $current_page = 0 )
	{
		...
		// Page title
		if ( '' === $page->post_title )
			$page->post_title = sprintf( __( '#%d (no title)', 'fitwp' ), $page->ID );
		$title = apply_filters( 'the_title', $page->post_title, $page->ID );
		if ( $has_dropdown )
			$title .= ' <b class="caret"></b>';

		$output .= '<li' . $css_class . '><a href="' . get_permalink( $page->ID ) . '"' . $attributes . '>' . $title . '</a>';
	}
}
