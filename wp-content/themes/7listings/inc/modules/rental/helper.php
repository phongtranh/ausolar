...
class Sl_Rental_Helper
{
	 ...
	 * @var string
	 */
	static $post_type = 'rental';

	/**
	 * This function gets all dates are 'unbookable' for an rental resource
	 *
	 * @param int    $post_id
	...
	static function get_unbookable_dates( $post_id = 0, $resource, $index = null, $date_format = 'd/m/Y' )
	{
		...
		{
			$checkin  = strtotime( str_replace( '/', '-', get_post_meta( $booking->ID, 'checkin', true ) ) );
			$checkout = strtotime( str_replace( '/', '-', get_post_meta( $booking->ID, 'checkout', true ) ) );
	...
	static function send_booking_emails( $booking_post_id, $type_action = 'all', $custom_email = '' )
	{
		...
		elseif ( has_post_thumbnail( $post_id ) )
			$resource_photo = sl_broadcasted_thumbnail( 'sl_pano_small', array(), $post_id, false );
		...
		$replacements = apply_filters( 'sl_email_shortcodes_rental', $replacements, $booking_post_id, $post, $resource );
	...
	{
		if ( ! isset( $resource['price'] ) || ! is_array( $resource['price'] ) )
			return false;

		// Remove empty prices
		$prices = array_filter( $resource['price'] );
