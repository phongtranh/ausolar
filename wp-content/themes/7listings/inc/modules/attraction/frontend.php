...
class Sl_Attraction_Frontend extends Sl_Core_Frontend
{
	/**
	 * Change meta title
	 *
	...
	function meta_title( $title = '', $sep = '' )
	{
		...
		$taxonomies = array( 'feature', 'attraction_type' );
	...
	public static function post_list( $args )
	{
		...
		$class = 'sl-list posts attractions';
		...
		if ( $args['more_listings'] )
		{
			...
			$link = get_post_type_archive_link( 'attraction' );

			// Fix ugly post type archive link
			if ( strpos( $link, '?' ) )
				$link = home_url( sl_setting( 'attraction_base_url' ) . '/' );

			// If set 'type' taxonomy, get link to that taxonomy page
			if ( $args['type'] )
			{
				$term = get_term( absint( $args['type'] ), sl_meta_key( 'tax_type', 'attraction' ) );
				if ( ! is_wp_error( $term ) )
				{
					...
					$term_link = get_term_link( $term, sl_meta_key( 'tax_type', 'attraction' ) );
	...
	/**
	 * Get dropdown list for attraction menu item
	 *
	 * @param string $ul
	 *
	 * @return string
	...
	function menu_dropdown( $ul )
	{
		...
		{
			// Get only locations of attraction
			global $wpdb;
			$query     = "SELECT t.term_id FROM $wpdb->terms AS t
				INNER JOIN $wpdb->term_taxonomy AS tt ON t.term_id = tt.term_id
				INNER JOIN $wpdb->term_relationships AS r ON r.term_taxonomy_id = tt.term_taxonomy_id
				INNER JOIN $wpdb->posts AS p ON p.ID = r.object_id
				WHERE p.post_type = 'attraction' AND tt.taxonomy = 'location'
				GROUP BY t.term_id";
...
new Sl_Attraction_Frontend( 'attraction' );
