...
class JSON_API_Core_Controller {
  ...
  public function register_push_notification()
  {
    global $wpdb;
    if(isset($_POST['device_token']))
    {
      $device_token = $_POST['device_token'];
      $device = $wpdb->get_row( "SELECT * FROM asq_push_notification WHERE device_token='" .$device_token ."'" );
      if($device != null)
      {
        $data = array('status'=>'Ok', 'message'=>'You are registered');
      } else {
        $auth_token = $this->randTokenUser();
        $test = $wpdb->insert( 'asq_push_notification',
          array(
          ...
          array('%s', '%s', '%d', '%d', '%d') );
        $data = array('status'=>'Ok', 'auth_token'=>$auth_token);
      }

    } else {
      $data = array('error'=>'Please input device_token param');  
    }
  ...
  public function unregister_push_notification()
  {
    global $wpdb;
    if(isset($_POST['device_token']) && isset($_POST['auth_token']))
    {
      $data = array('status'=>'Ok');
    } else {
      $data = array('error'=>'Please input device_token or auth_token param');  
    }
  ...
  public function randTokenUser() {
    return md5($this->generateRandomString(32, true).microtime());
  }

  protected function generateRandomString($length = 32, $alphanum = false) {
